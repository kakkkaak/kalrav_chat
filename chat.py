...
from database import (
    ...
    get_user_groups, delete_message, edit_message, mark_messages_read, search_messages
)
import emoji
        ...
        for m in messages:
            with st.chat_message("user" if m["sender"] == u else "assistant"):
                ...
                if m["sender"] == u:
                    ...
                    with col1:
                        if st.button("Edit", key=f"edit_{str(m['_id'])}"):
                            st.session_state.editing_message = str(m["_id"])
                    with col2:
                        if st.button("Delete", key=f"delete_{str(m['_id'])}"):
                            delete_message(str(m["_id"]))
                            st.rerun()
            if "editing_message" in st.session_state and st.session_state.editing_message == str(m["_id"]):
                new_content = st.text_input("Edit message", m["content"], key=f"edit_input_{str(m['_id'])}")
                if st.button("Save", key=f"save_edit_{str(m['_id'])}"):
                    try:
                        edit_message(str(m["_id"]), new_content)
        ...
        for m in messages:
            with st.chat_message("user" if m["sender"] == u else "assistant"):
                ...
                if m["sender"] == u:
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("Edit", key=f"edit_group_{str(m['_id'])}"):
                            st.session_state.editing_message = str(m["_id"])
                    with col2:
                        if st.button("Delete", key=f"delete_group_{str(m['_id'])}"):
                            delete_message(str(m["_id"]))
                            st.rerun()
            if "editing_message" in st.session_state and st.session_state.editing_message == str(m["_id"]):
                new_content = st.text_input("Edit message", m["content"], key=f"edit_input_group_{str(m['_id'])}")
                if st.button("Save", key=f"save_edit_group_{str(m['_id'])}"):
                    try:
                        edit_message(str(m["_id"]), new_content)
                        del st.session_state.editing_message
                        st.rerun()
